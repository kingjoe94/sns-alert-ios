...
### 目的
- 1日の合計使用時間ではなく、アプリの連続使用時間に対して通知する。

### 基本ルール
- 通知閾値は **アプリごとに設定可能**（初期値は `OFF`）。
- 閾値到達時は **ローカル通知のみ** を行い、ブロックは行わない。
- クールダウンは各アプリの通知閾値と同じ値を使う。
  - 例：通知閾値が 15 分なら、通知後 15 分は再通知しない。

### 連続セッションの定義
- 同一アプリの使用イベント（1分同期）が継続している間を同一セッションとみなす。
- 次の場合は新しいセッションとして扱い、連続使用時間をリセットする。
  - 監視対象の別アプリ使用イベントへ切り替わった場合
  - 一定時間（120秒）以上、同一アプリの使用イベントが途切れた場合
  - 日次リセット実行時
  - 監視の停止/再開始時

### 通知文言
- タイトル: `SNSアラート`
- 本文: アプリ番号・連続使用分・通知閾値を含む簡易メッセージ
